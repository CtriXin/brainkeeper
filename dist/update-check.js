...
export async function maybePrintUpdateNotice() {
    ...
    if (isNewer(latest, current))
        maybeWriteNotice(cachePath, nextCache, current, latest, now);
}
function shouldSkipUpdateCheck() {
    ...
    if (process.env.NO_UPDATE_NOTIFIER)
        return true;
    if (process.env.BRAINKEEPER_NO_UPDATE_CHECK)
        return true;
    if (process.env.BK_NO_UPDATE_CHECK)
        return true;
    return false;
}
...
function maybeWriteNotice(path, cache, current, latest, now) {
    if (cache.noticeShownAt && now - cache.noticeShownAt < NOTICE_INTERVAL_MS)
        return;
